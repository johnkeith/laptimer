//
//  constnats.swift
//  LapTimer
//
//  Created by John Keith on 11/14/16.
//  Copyright © 2016 John Keith. All rights reserved.
//

import Foundation
import UIKit

struct Constants {
    static let colorPalette: [String: UIColor] = [
        "blue-background": UIColor(red:0.01, green:0.53, blue:0.82, alpha:1.0),
        "gray": UIColor(red:0.67, green:0.73, blue:0.79, alpha:1.0),
        "light-orange": UIColor(red:0.97, green:0.36, blue:0.30, alpha:1.0),
        "dark-blue": UIColor(red:0.00, green:0.28, blue:0.56, alpha:1.0),
        "dark-gray": UIColor(red:0.42, green:0.48, blue:0.54, alpha:1.0),
        "dark-orange": UIColor(red:0.84, green:0.24, blue:0.17, alpha:1.0)
    ]
//    static let colorPalette: [String: UIColor] = [
//        "blue-background": UIColor(red:0.07, green:0.07, blue:0.07, alpha:1.0),
//        "gray": UIColor(red:0.42, green:0.42, blue:0.42, alpha:1.0),
//        "dark": UIColor(red:0.18, green:0.18, blue:0.16, alpha:1.0),
//        "light-orange": UIColor(red:0.20, green:0.30, blue:0.56, alpha:1.0),
//        "light-blue": UIColor(red:0.36, green:0.77, blue:0.94, alpha:1.0),
//        "light-orange": UIColor(red:1.00, green:0.72, blue:0.27, alpha:1.0),
//        "dark-orange": UIColor(red:0.91, green:0.33, blue:0.17, alpha:1.0)
//    ]
}
